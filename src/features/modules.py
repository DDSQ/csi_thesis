...
def tree_imp(df,cols):
    ...
    #assign the forrest
    clf_ens = ExtraTreesClassifier(random_state = 2018)
...
def feat_plot(df,cols, title):
    ...
    ax = feat_df.plot(kind = 'barh', title = title, xlim = (0,1))
...
def score_knn_cv(df,cols):
    ...
    score_list_k = pd.DataFrame({'scores': [i for i in score_list ], 'K': [int(i) for i in y if i%2 == 1]})
    ...
    value_of_best_k = int(score_list_k[score_list_k.scores == np.amax(score_list_k.scores)].K)
        
    return score_list_k, value_of_best_k
...
def ensemble_classifier(df,cols):
    ...
    clf_ens = ExtraTreesClassifier(bootstrap = True, oob_score = True, random_state = rs, verbose =3)
    grid_clf_ens = GridSearchCV(clf_ens, param_grid = param_grid_extra , cv = StratifiedKFold(n_splits = 5, shuffle = True, random_state = rs)).fit(X_train, y_train)
    
    #assign the forrest_model and train
    clf_for = RandomForestClassifier(bootstrap = True, oob_score = True, random_state = rs, verbose = 3)
    grid_clf_for = GridSearchCV(clf_for, param_grid = param_grid_extra , cv = StratifiedKFold(n_splits = 5, shuffle = True, random_state = rs)).fit(X_train, y_train)
    ...
    y_hat_forest = grid_clf_for.best_estimator.predict(X_test)
...
def compute_rel_red_cols(df):
    ...
    df2.experience_x = df2[['experience_x']].astype(int)
    df2.rename( columns = {'experience_x': 'experience'})
